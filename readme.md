...
### 1. index 페이지 클러스트 지도 팝업 개선

- 클러스트 지도 팝업에 캠프장 이름, 설명, 이미지 추가
- 캠프장 이름에 상세 페이지(show 페이지) 링크 연결
- 클러스트 지도 팝업 스타일 개선

### 2. 캠프그라운드 삭제 시 Cloudinary 파일도 함께 삭제

- 삭제 미들웨어 추가

### 3. 이미지 유효성 검사 추가 (서버/클라이언트)

- 파일 타입, 파일 개수, 파일 용량 제한
...
3. **React 전환**: 프론트엔드를 React로 마이그레이션
4. **유저 신규 가입 시 동일 페이지에서 아이디, 이메일 중복 확인**
5. **신규 가입 시 이메일 검증**: 이메일 보내서 재접속 확인
6. **유저정보 페이지 추가**: 이메일, 비밀번호 변경 가능, 본인이 작성한 캠프장 및 리뷰 조회
7. **OAuth 가입**: Google, Facebook 등을 통한 인증
8. **사진 업로드 관리 도구 추가**: 드래그 앤 드롭, 사진 추가/삭제, 미리보기 등
9. **리뷰 내용 업데이트 기능**
10. **주소 입력 시 실시간 지도 표시 및 선택 기능**
